...
SiteDelivery = {
	...
	ydelivery: {
		...
		courrier: {
			title: 'Курьерская доставка до подъезда'
		},
		pickup: {
			title: 'Самовывоз со склада в вашем городе',
			url: 'https://yadostavkains.pimentos.net/delivery/get_pickup_data?insales_id=******'
		}
	}
}
...
SiteDelivery.ydelivery.courrier.decorate = function(params){
	...
	for(var i = 0, length1 = Deliveries[id].subdeliveries.length; i < length1; i++){
		...
		<div class="row">\
		<div class="js-geo-title geo-table-item-title">'+ delivery.title +'</div>\
		<div class="js-geo-price geo-table-item-price">' + InSales.formatMoney(delivery.price, cv_currency_format) + '</div>\
		<div class="js-geo-terms geo-table-item-terms">' + days + '</div>\
		<div class="js-geo-payments geo-table-item-payments">' + payments.join('') + '</div>\
		</div>\
		</div>';

		line_mini = '\
		<li class="geo-mini-table-item js-geo-item-' +id+ '' + delivery.tariff_id + '">\
		<span class="js-geo-title">' + delivery.title + '</span>\
		<span>\
		<span>\
...
SiteDelivery.processed_variants = function (e) {
	...
	ddLineDefault = '<div class="geo-table-title hidden-xs"><div class="row"><div class="geo-table-item-title">Варианты доставки</div><div class="geo-table-item-price">Стоимость</div><div class="geo-table-item-terms">Сроки</div><div class="geo-table-item-payments">Варианты оплаты</div></div></div>';
	ddLineMini = '';
	...
	$.each(e.deliveries, function(index, item){
		...
		ddLineDefault += '<div class="geo-table-item js-geo-item-'+item.id+'"><div class="row"><div class="js-geo-title geo-table-item-title">'+$title+'</div><div class="js-geo-price geo-table-item-price">'+InSales.formatMoney(item.price, cv_currency_format)+'</div><div class="js-geo-terms geo-table-item-terms">&nbsp;</div><div class="js-geo-payments geo-table-item-payments">&nbsp;</div></div></div>';
		ddLineMini += '<li class="geo-mini-table-item js-geo-item-'+item.id+'"><span class="js-geo-title">'+item.title+': </span> <span><span class="geo-price"><span class="js-geo-price">'+InSales.formatMoney(item.price, cv_currency_format)+'</span><span class="js-geo-terms">&nbsp;</span></stron></span></li>'
	});
	...
	$.each(geoMyCityes, function(index,item){
		if(country == index){
			...
			$.each(item, function(index,item){
				if(item.indexOf('*') != -1){
					$populars += '<div class=""><strong><span class="js-geo-popular-item geo-popular-item" data-popular="'+$country+'|'+index+'|'+item.replace('*','')+'">'+item.replace('*','')+'</span></strong></div>';
				}else{
					$populars += '<div class=""><span class="js-geo-popular-item geo-popular-item" data-popular="'+$country+'|'+index+'|'+item.replace('*','')+'">'+item.replace('*','')+'</span></div>';
				}
			});
			$populars += '</div></div>';
			$populars += '</div>';
...
function checkDelivery(country, region, city){
	...
	if(country == 'RU'){
		...
		if(geoIsUpdate){
          if(geo_active){
			$('.js-geo-city').html('<span>Доставка в город:</span> <a href="'+geoDeliveryPageUrl+'" class="winbox" data-window="geo|geoCity">'+city+'</a><span class="js-geo-minterms"></span><span class="js-geo-main" data-geo-main="'+country+'|'+region+'|'+city+'"></span>');
          } else {
			$('.js-geo-city').html('<span>Доставка в город:</span> '+city+'<span class="js-geo-minterms"></span><span class="js-geo-main" data-geo-main="'+country+'|'+region+'|'+city+'"></span>');            
          }
